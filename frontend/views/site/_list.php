...
<?php if($model->filename !== '' && !empty($model->filename)){ ?>
	<?php if(Yii::$app->user->isGuest){ ?>
		<li class="awp-playlist-item" data-tone="<?php echo $tone;?>" data-bpm="<?php echo $bpm;?>" data-type="audio" data-mp3="<?php echo '/audio/min/'.$name;?>" data-title="<?=$model->filename?> <br><span class='bpm-n-tone'><?php echo $tone_n_bpm_string;?></span> " data-download="/site/about/"></li> 
	<?php } elseif(Yii::$app->user->identity->premium > 0){ ?>
		<li class="awp-playlist-item" <?php echo $is_dld;?> <?php echo $isnew;?> data-tone="<?php echo $tone;?>" data-bpm="<?php echo $bpm;?>" data-type="audio" data-mp3="<?=$model->path?>" data-title="<?=$model->filename?> <br><span class='bpm-n-tone'><?php echo $tone_n_bpm_string;?></span> " data-download="<?=$model->path?>"></li> 
	<?php } else { ?>
		<li class="awp-playlist-item" <?php echo $isnew;?> data-tone="<?php echo $tone;?>" data-bpm="<?php echo $bpm;?>" data-type="audio" data-mp3="<?php echo '/audio/min/'.$name;?>" data-title="<?=$model->filename?> <br><span class='bpm-n-tone'><?php echo $tone_n_bpm_string;?></span> " data-download="/site/about/"></li> 
	<?php } ?>
<?php } ?>